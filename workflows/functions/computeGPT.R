computeGPT=function(input_path, output_path, gpt_exe, graphxml_filepath){
  ...
  graphXml_files = list.files(graphxml_filepath,
                              pattern=glob2rx("*.xml"),
                              full.names = TRUE)
  ...
  for(infile in input_files){
    ...
    for(xml_file in graphXml_files){
      ...
      cmd = paste0(gpt_exe, " ", xml_file, " -e -t ", outfile, " -Pinfile=", infile)
      print(cmd)
      system(cmd)
